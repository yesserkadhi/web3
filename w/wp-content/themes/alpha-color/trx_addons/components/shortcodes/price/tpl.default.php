...
?><div <?php if (!empty($args['id'])) echo ' id="'.esc_attr($args['id']).'"'; ?> 
	...
	foreach ($args['prices'] as $item) {
		...
		?>>
			...
			?><div class="sc_price_item_info"><div class="price-header"><?php
				...
				if (!empty($item['description'])) {
					...
					?><div class="sc_price_item_description"><?php
						foreach ($item['description'] as $str) {
							?><span><?php echo wp_kses(trx_addons_parse_codes($str), 'alpha_color_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				?></div><div class="price-body"><?php
				if (!empty($item['price'])) {
					?><div class="sc_price_item_price"><?php
						if (!empty($item['before_price'])) {
							?><span class="sc_price_item_price_before"><?php echo wp_kses(trx_addons_parse_codes($item['before_price']), 'alpha_color_kses_content'); ?></span><?php
						}
						?><span class="sc_price_item_price_value"><?php echo wp_kses($item['price'], 'alpha_color_kses_content'); ?></span><?php
						if (!empty($item['after_price'])) {
							?><span class="sc_price_item_price_after"><?php echo wp_kses(trx_addons_parse_codes($item['after_price']), 'alpha_color_kses_content'); ?></span><?php
						}
					?></div><?php
				}
				...
				if (!empty($item['link']) && !empty($item['link_text'])) {
					?><a href="<?php echo esc_url($item['link']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_price_item_link sc_button', 'sc_price', $args)); ?>"><?php
						echo esc_html($item['link_text']);
					?></a><?php
				}
			?></div></div><!-- /.sc_price_ite_info --><?php
			if (!empty($item['bg_image']) || !empty($item['bg_color'])) {
				?></div><!-- /.sc_price_item_inner --><?php
			}
			// Link over whole block - if 'limnk' is not empty and 'link_text' is empty
			if (!empty($item['link']) && empty($item['link_text'])) {
