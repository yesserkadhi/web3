...
class Essential_Grid_Admin extends Essential_Grid_Base
{
	...
	{
		global $EssentialAsTheme, $esg_dev_mode, $wp_version, $pagenow;
	...
	public static function update_create_grid($data)
	{
		...
		$stream_limit = self::get_stream_count_limit();
		$limit_params = array('youtube-count', 'vimeo-count', 'instagram-count', 'flickr-count', 'facebook-count', 'twitter-count', 'behance-count');
		foreach ($limit_params as $p) {
			if ($data['postparams'][$p] > $stream_limit) $data['postparams'][$p] = $stream_limit;
		}
			...
			if ($isVerified) {
				...
				switch ($clientAction) {
					...
					case 'update_general_settings':
						...
						update_option('tp_eg_wait_for_fonts', self::getVar($data, 'wait_for_fonts'));
					...
					case 'import_default_post_data':
						try {
							...
							if (isset($tp_grid_meta_fonts)) {
								...
								$custom_fonts = self::getVar($tp_grid_meta_fonts, 'punch-fonts');
					...
					case 'import_data':
						...
						try {
							...
							$custom_fonts = self::getVar($data, array('imports', 'data-punch-fonts'), array(), 'r');
							if (!empty($custom_fonts) && is_array($custom_fonts)) {
								...
								if (!empty($custom_fonts)) {
									$custom_fonts_handle = self::getVar($data, array('imports', 'import-punch-fonts-handle'));
					...
					case "get_youtube_playlists":
						if (!empty($data['api'])) {
							$playlist = self::getVar($data, 'playlist');
					...
					case "get_flickr_photosets":
						if (!empty($data['url'])) {
							...
							$return = $flickr->get_photo_sets($user_id, $data['set']);
					...
					case "get_ids_by_data":
						if (!empty($data)) {
							$types = self::getPostVar('data', array());
							$ret_ids = array();
							...
							foreach ($types as $type => $values) {
								switch ($type) {
									case 'posts':
										...
										$cat_tax = self::getCatAndTaxData($values['post_category']);
										$page_ids = explode(',', self::getVar($values, 'selected_pages'));
										$additional_query = wp_parse_args($values['additional_query']);
										$ids = self::getPostIdByCategory($cat_tax['cats'], $values['post_types'], $cat_tax['tax'], $page_ids, $sortBy = 'ID', $direction = 'DESC', $values['max_entries'], $additional_query, false, $values['post_relation']);
										$ret_ids['posts'] = $ids;
										break;
									default:
										break;
					...
					case "load_specific_items_markup":
						$gridid = self::getPostVar('gridid', 0, 'i');
					...
					case 'install_addon':
						...
						if ($return === true) {
							...
							Essential_Grid::ajaxResponseError($error);
						}
						break;
					...
					case 'deactivate_addon':
						$handle = self::getVar($data, 'addon');
						$esg_addons = Essential_Grid_Addons::instance();
						$return = $esg_addons->deactivate_addon($handle);

						if ($return) {
							do_action('essgrid_deactivate_addon', $handle);
							Essential_Grid::ajaxResponseSuccess(__('AddOn deactivated', ESG_TEXTDOMAIN));
						} else {
							Essential_Grid::ajaxResponseError(esc_attr__('AddOn could not be deactivated!', ESG_TEXTDOMAIN));
						}
						break;
					...
					case 'disable_addon':
						...
						if ($return) {
							...
							Essential_Grid::ajaxResponseError(esc_attr__('AddOn could not be disabled!', ESG_TEXTDOMAIN));
						}
						break;
					...
					case 'check_for_updates':
						...
						if ($version !== false) {
							...
							Essential_Grid::ajaxResponseError(esc_attr__('Connection to Update Server Failed!', ESG_TEXTDOMAIN));
						}
						break;
						...
						if ($return) {
							if (is_array($return)) {
								if (isset($return['error'])) {
									Essential_Grid::ajaxResponseError($return['error']);
								}
