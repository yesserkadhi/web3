...
if (!function_exists('trx_addons_sc_blogger_add_in_vc_params')) {
	function trx_addons_sc_blogger_add_in_vc_params() {
		...
		// Prepare lists
		$post_type = $vc_edit && !empty($vc_params['post_type']) ? $vc_params['post_type'] : 'product';
		$taxonomy = $vc_edit && !empty($vc_params['taxonomy']) ? $vc_params['taxonomy'] : 'category';
		$tax_obj = get_taxonomy($taxonomy);
		return apply_filters('trx_addons_sc_map', array(
				"base" => "trx_sc_blogger",
				"name" => esc_html__("Blogger", 'trx_addons'),
				"description" => wp_kses_data( __("Display products from specified category in many styles", 'trx_addons') ),
    ...
    function trx_addons_sc_blogger_add_in_elementor() {
        ...
        class TRX_Addons_Elementor_Widget_Blogger extends TRX_Addons_Elementor_Widget {
            ...
            protected function register_controls() {
                ...
                // Prepare lists
                $post_type = !empty($params['post_type']) ? $params['post_type'] : 'product';
...
//                        'show_label' => true,
//                        'type' => \Elementor\Controls_Manager::SELECT,
//                        'options' => apply_filters('trx_sc_blogger', trx_addons_components_get_allowed_layouts('sc', 'blogger'), 'trx_sc_blogger'),
//                        'default' => 'default',
//                    ]
//                );
