...
if ( !function_exists( 'trx_addons_cv_resume_merge_styles' ) ) {
	add_filter("trx_addons_filter_merge_styles", 'trx_addons_cv_resume_merge_styles');
...
if (!function_exists('trx_addons_cv_resume_show_posts')) {
	function trx_addons_cv_resume_show_posts($slug, $page=1) {
		...
		if ($query->found_posts > 0) {
			...
			?>
			<div class="trx_addons_cv<?php echo esc_attr($prn); ?>_resume trx_addons_cv<?php echo esc_attr($prn); ?>_resume_style_<?php echo esc_attr($style); ?><?php if ($slider) echo ' slider_container swiper-slider-container slider_swiper slider_noresize slider_controls slider_controls_bottom'; ?><?php if (trx_addons_is_on(trx_addons_get_option('cv_resume_print_full'))) echo ' trx_addons_cv'.esc_attr($prn).'_resume_text_full'; ?>"
					<?php
					if ( empty($prn) ) {						// Not Print version
				...
				if ( empty($prn) ) {							// Not Print version
					if ($slider) {
						?><div class="slides slider-wrapper swiper-wrapper"><?php
					} else if ($columns > 1) {
